\name{TbLatex}
\alias{TbLatex}
\title{Sampling Tables in LaTeX}
\description{Generates a TeX file containing survey sampling tables}
\usage{
TbLatex(Folder, Table, Estimation, Camp, Label, Title, Header, Note, Ap)
}
\arguments{
\item{Folder}{The place where you want to save the generated TeX file. The folder must already be created in advance.}
\item{Table}{A DataFr object. Indicates all of the the tables to include in the TeX file.}
\item{Estimation}{A DataFr object. Includes the estimated values of the characteristics of interest for the whole population and
the domains of interest, as with its respective coefficients of variation.}
\item{Camp}{A DataFr object. Gives the identification number, the type and the name of every characteristic of interest
to include in the tables generated by the TeX file.}
\item{Label}{A DataFr object. Indicates the categories for every domain of interest.}
\item{Title}{A DataFr object. Tables titles in the TeX file.}
\item{Header}{A DataFr object. Tables headers in the TeX file.}
\item{Note}{A DataFr object. Tables Footnotes in the TeX file.}
\item{Ap}{Appearance of the generated TeX file. By Default, Ap=0 generates a vertical document. However Ap=1
generates a landscape document.}
}
\seealso{
\code{\link{DataFr}}
}
\value{The function returns a TeX file containing survey smapling tables. When this file is created, 
the fucntion returns the folder used to store the TeX file, and the name of the file. 
By default, the corresponding file name is Tables\_0.tex.}
...
# Folder = "c:/temp"
TbLatex(Folder, Table, Estimation, Camp, Label, Title, Header, Note, 0)
# STOP: Now... GO to the folder "c:/temp" and compile 
# the created file in your prefered LaTeX editor !!!!
